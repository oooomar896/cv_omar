      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <AnimatePresence mode='wait'>
          {NEWS_ITEMS.map((newsItem, index) => {
            ...
            const CardIcon = newsItem.cardIcon;
            ...
            return (
              ...
              >
                <div className='text-center'>
                  <h2 className='section-title'>{newsItem.sectionTitle}</h2>
                  <p className='text-xl text-gray-400 max-w-3xl mx-auto'>
                    {newsItem.intro}
                  </p>
                  <div className='mt-6 flex justify-center space-x-3 space-x-reverse'>
                    {NEWS_ITEMS.map((item, buttonIndex) => (
                      <button
                        key={item.id}
                        onClick={() => setActiveIndex(buttonIndex)}
                        className={`h-2 w-8 rounded-full transition-all duration-300 ${
                          buttonIndex === activeIndex
                            ? 'bg-primary-500 w-12'
                            : 'bg-gray-700 hover:bg-primary-400'
                        }`}
                        aria-label={`عرض الخبر رقم ${buttonIndex + 1}`}
                      />
                    ))}
                  </div>
                </div>
                      ...
                      >
                        <img
                          ...
                          className={`w-full h-80 ${
                            newsItem.image.className || 'object-cover'
                          }`}
                        />
                        <div className='absolute inset-0 bg-gradient-to-t from-black/50 to-transparent' />
                        <div className='absolute bottom-4 left-4 right-4'>
                          <h5 className='text-white font-bold text-lg mb-2'>
                            {newsItem.image.overlayTitle}
                          </h5>
                          <p className='text-gray-200 text-sm'>
                            {newsItem.image.overlaySubtitle}
                          </p>
                        </div>
                      </motion.div>
                    </div>
